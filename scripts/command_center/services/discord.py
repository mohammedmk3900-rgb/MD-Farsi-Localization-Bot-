...
def stats_embed(config: Config, payload: dict) -> dict:
    ...
    return {
        "author": {"name": "MD FARSI LOCALIZATION • COMMAND CENTER V6"},
...
def progress_embed(config: Config, payload: dict) -> dict:
    ...
    return {
        ...
        "footer": {"text": f"ParaTranz Project {config.project_id} • Automation V6"},
        "timestamp": now.isoformat(),
    }
...
def health_embed(config: Config, health: dict) -> dict:
    ...
    return {
        ...
        "fields": fields,
        "footer": {"text": "MD Farsi Localization • V6 Health Engine • بدون نمایش Secret"},
