...
def request_json(url, method="GET", payload=None, headers=None, retries=3, timeout=30):
    base={"Accept":"application/json","User-Agent":"MD-Farsi-Localization-Command-Center/5.0"}
    ...
    for attempt in range(retries):
        try:
            ...
            if exc.code not in RETRYABLE or attempt >= retries-1: raise last
            delay=float(retry_after) if retry_after else 2**attempt
            time.sleep(min(delay,30))
        except (URLError, TimeoutError, json.JSONDecodeError) as exc:
            last=RuntimeError(str(exc))
            if attempt >= retries-1: raise last
