...
if not TOKEN or not GUILD_ID: raise RuntimeError("DISCORD_BOT_TOKEN and DISCORD_GUILD_ID are required")
index=MessageIndex(DB_PATH)

def serialize(message: discord.Message)->dict[str,Any]:
    return {"id":str(message.id),"channel_id":str(message.channel.id),"channel_name":getattr(message.channel,"name",None),
            ...
            "content":message.content or "","timestamp":message.created_at.isoformat(),
            "edited_timestamp":message.edited_at.isoformat() if message.edited_at else None,"url":message.jump_url}
...
class IndexerBot(discord.Client):
    def __init__(self)->None:
        ...
        super().__init__(intents=intents); self._backfill_started=False

    async def on_ready(self)->None:
        guild=self.get_guild(GUILD_ID)
        if guild is None:
            log.error("Guild %s is not visible to the bot; check guild ID and membership.",GUILD_ID); return
        log.info("Connected as %s; guild=%s (%s)",self.user,guild.name,guild.id)
        if BACKFILL_ON_START and not self._backfill_started:
            self._backfill_started=True; asyncio.create_task(self.backfill_guild(guild))

    async def backfill_guild(self,guild:discord.Guild)->None:
        channels=[c for c in guild.channels if is_indexable(c)]
        ...
        async def one(channel:object)->None:
            async with sem:
                try: await self.backfill_channel(channel) # type: ignore[arg-type]
                except (discord.Forbidden,discord.NotFound) as exc: log.warning("Skipping inaccessible channel %s: %s",getattr(channel,"id","?"),exc)
                except discord.HTTPException as exc: log.warning("Discord API error for channel %s: %s",getattr(channel,"id","?"),exc)
                except Exception: log.exception("Unexpected backfill error for channel %s",getattr(channel,"id","?"))
